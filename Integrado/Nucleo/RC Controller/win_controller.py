...
def mixAngularChannels(input):
    if input < dead_zone_size and input > 0:
        ...
        mixedAngularSpeed = round(
            map(input, dead_zone_size, max_input, 0, 1), 2)
    else:
        mixedAngularSpeed = round(
            map(input, -max_input, -dead_zone_size, -1, 0), 2)
    ...
    def run(self):		# Função default invocada pela biblioteca Thread
        while True:
            try:
                ...
                self.linearSpeed = mixLinearChannels(
                    self.RTrigger, self.LTrigger)
                self.angularSpeed = mixAngularChannels(self.LStickX)

            # Lida com a exceção do controle estar desconectado
            except UnpluggedError:
                clearConsole()
                print("Controle desconectado...\nConecte o controle e reinicie o programa")
                exit()
